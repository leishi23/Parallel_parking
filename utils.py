...
class DataLogger:
    ...
    def save_data(self):
        ...
        # plot error
        plt.figure(figsize=(12,12)) 
        plt.scatter(t, np.linalg.norm(self.car_state[:,:2]-self.path, axis=1), color='black', s=8)
        plt.title('car\'s position error in time',fontsize=20)
        plt.xlabel('time (s)',fontsize=20)
        plt.ylabel('error (m)',fontsize=20)
        plt.grid()
        plt.legend(['error'], prop=font) # using a named size
        plt.savefig('log results/error.png')
